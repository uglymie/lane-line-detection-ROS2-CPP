...
void LaneLineDetection::initNode()
{
  ...
  line_pub = this->create_publisher<lane_line_msgs::msg::Line>(
      "/line/info", 1000);
}
...
void LaneLineDetection::imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
{
  ...
  lane_line_msgs::msg::Line line_msg;
  line_msg.curvature = curvature;
  line_msg.deviate = vehicle_offset;
...
void LaneLineDetection::detectionForward(const Mat &src, Mat &dst)
{
  ...
  findLine(th_img, lp, rp, rightx_current, leftx_current);
  drawArea(un_img, dst, lp, rp, Minv);
}
